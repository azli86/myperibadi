...
## Already Installed (do not run the install wizard)

All six skills live in `.pi/skills/` in this repo: `antislop` (core), `antislop-ui`, `antislop-copywriting`, `antislop-human`, `antislop-layoutmobile`, `antislop-code`. The upstream wizards, installers and pointer blocks are not used here.

Project direction already exists — read it instead of inventing one:
- Design tokens (light + dark) live in `apps/web/src/app/globals.css`. Use the `--*` variables; never hardcode colors.
- Established patterns: `apps/web/src/components/ui/`, `apps/web/src/components/layout/PageHeader.tsx` (`MobilePageHeader`, `DesktopPageHeader`), `AppSheetHeader`, native `<select>`.
- Copy language: Malay first, English secondary via the `lang` prop (`apps/web/src/lib/lang.tsx`).

When to apply the filter: ask the user once whether to apply it while, or after, the work. Default for small edits in this repo: apply silently while working, then report what was filtered.
