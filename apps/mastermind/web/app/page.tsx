...
type View = "dashboard" | "users" | "households" | "logs" | "analytics" | "system" | "activity" | "transactions" | "tickets";
type Stats = Record<"users"|"active_users"|"households"|"transactions"|"transactions_30d"|"active_wallets"|"requests_24h"|"new_users_7d"|"active_sessions_24h"|"total_income"|"total_expense"|"total_net"|"total_amount", number>;
...
export default function Home() {
  ...
  async function showTickets(kind="",status="",q="",append=false){setView("tickets");setError("");const r=await fetch(`/api/support/tickets?kind=${encodeURIComponent(kind)}&status=${encodeURIComponent(status)}&q=${encodeURIComponent(q)}&offset=${append?tickets.length:0}&limit=50`,{credentials:"include"});if(!r.ok){setError("Tiket gagal dimuat");return}const d=await r.json();if(append)setTickets(t=>[...t,...d.tickets]);else setTickets(d.tickets);setTicketsTotal(d.total)}
  async function openTicket(id:number){const r=await fetch(`/api/support/tickets/${id}`,{credentials:"include"});if(!r.ok){setError("Detail tiket gagal dimuat");return}setTicketDetail(await r.json())}
  async function setTicketStatus(id:number,status:string){if(!window.confirm("Tukar status tiket ini?"))return;setError("");const r=await fetch(`/api/support/tickets/${id}/status`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({status})});if(!r.ok){const d=await r.json().catch(()=>null);setError((d&&d.detail)||"Tindakan gagal");return}await openTicket(id);await showTickets()}
  ...
  return <div className="shell"><aside className="side"><div className="brand">Mastermind</div><small>MyPeribadi Admin</small><nav className="nav"><button className={view==="dashboard"?"active":""} onClick={()=>setView("dashboard")}>Dashboard</button><button className={view==="users"?"active":""} onClick={()=>void showUsers()}>Pengguna</button><button className={view==="households"?"active":""} onClick={()=>void showHouseholds()}>Household</button><button className={view==="analytics"?"active":""} onClick={()=>void showAnalytics()}>Statistik</button><button className={view==="activity"?"active":""} onClick={()=>void showActivity()}>Aktiviti</button><button className={view==="transactions"?"active":""} onClick={()=>void showTransactions()}>Transaksi</button><button onClick={()=>{window.location.href="/live"}}>Live Feed</button><button className={view==="system"?"active":""} onClick={()=>void showSystem()}>Sistem</button><button className={view==="logs"?"active":""} onClick={()=>void showLogs("login")}>Log</button><button className={view==="tickets"?"active":""} onClick={()=>void showTickets()}>Tiket</button></nav></aside><main className="main"><header className="top"><div><h1>{{dashboard:"Ringkasan sistem",users:"Pengguna",households:"Household",logs:"Log",analytics:"Statistik",system:"Sistem",activity:"Aktiviti Terkini",transactions:"Transaksi Terkini",tickets:"Request & Tiket"}[view]}</h1><p className="muted">Akses pentadbiran MyPeribadi.</p></div><button className="primary" onClick={async()=>{await fetch("/api/auth/logout",{method:"POST"});setAuthenticated(false)}}>Log keluar</button></header>{error&&<p className="error">{error}</p>}
  {view==="dashboard"&&<>
    <div className="hero">
      <div className="hero-label">Jumlah Wang Direkod</div>
      <div className="hero-amount">RM {Number(stats?.total_amount||0).toLocaleString("ms-MY",{maximumFractionDigits:0})}</div>
      <div className="hero-sub">
        <span className="inc">▲ Pendapatan <b>RM {Number(stats?.total_income||0).toLocaleString("ms-MY",{maximumFractionDigits:0})}</b></span>
        <span className="exp">▼ Perbelanjaan <b>RM {Number(stats?.total_expense||0).toLocaleString("ms-MY",{maximumFractionDigits:0})}</b></span>
        <span className={"net "+(Number(stats?.total_net||0)<0?"neg":"pos")}>Bersih <b>RM {Number(stats?.total_net||0).toLocaleString("ms-MY",{maximumFractionDigits:0})}</b></span>
      </div>
    </div>
  ...
  {view==="tickets"&&<><div className="card">
    <div className="row" style={{justifyContent:"space-between"}}>
      <div className="row" style={{gap:"6px"}}>
        {[["","Semua"],["feature","Feature"],["support","Support"],["bug","Bug"]].map(([k,l])=><button key={k} className="tab" onClick={()=>void showTickets(k as string)}>{l}</button>)}
      </div>
      <span className="muted">{tickets.length} / {ticketsTotal} tiket</span>
    </div>
    <table className="table"><thead><tr><th>Jenis</th><th>Tajuk</th><th>Pengguna</th><th>Status</th><th>Prioriti</th><th>Dihantar</th><th></th></tr></thead><tbody>{tickets.map(t=><tr key={t.id}><td><span className="pill">{t.kind}</span></td><td><strong>{t.title}</strong>{t.description&&<><br/><span className="muted">{t.description.length>80?t.description.slice(0,80)+"…":t.description}</span></>}</td><td><strong>{t.user_name||"—"}</strong><br/><span className="muted">{t.user_email}</span></td><td><span className={"pill "+(t.status==="resolved"?"ok":t.status==="closed"?"bad":t.status==="in_progress"?"":"warn")}>{t.status}</span></td><td><span className={"pill "+(t.priority==="high"?"bad":t.priority==="low"?"ok":"warn")}>{t.priority}</span></td><td>{fmtDate(t.created_at)}</td><td><button className="ghost" onClick={()=>void openTicket(t.id)}>Detail</button></td></tr>)}</tbody></table>
    {tickets.length<ticketsTotal&&<div style={{textAlign:"center",marginTop:"12px"}}><button className="primary" onClick={()=>void showTickets("","","",true)}>Muat lagi</button></div>}
  </div>
  {ticketDetail&&<div className="overlay" onClick={()=>setTicketDetail(null)}><div className="modal" onClick={e=>e.stopPropagation()}><div className="modal-head"><h2>#{ticketDetail.id} · {ticketDetail.title}</h2><button className="ghost" onClick={()=>setTicketDetail(null)}>Tutup</button></div>
    <div className="kv"><span>Jenis</span><b>{ticketDetail.kind}</b></div>
    <div className="kv"><span>Pengguna</span><b>{ticketDetail.user_name} · {ticketDetail.user_email}</b></div>
    <div className="kv"><span>Prioriti</span><b>{ticketDetail.priority}</b></div>
    <div className="kv"><span>Status</span><b>{ticketDetail.status}</b></div>
    <div className="kv"><span>Dihantar</span><b>{fmtDate(ticketDetail.created_at)}</b></div>
    {ticketDetail.resolved_at&&<div className="kv"><span>Diselesaikan</span><b>{fmtDate(ticketDetail.resolved_at)}</b></div>}
    <p style={{marginTop:"14px"}}>{ticketDetail.description||"—"}</p>
    <div className="row" style={{marginTop:"16px",gap:"8px"}}>
      {[["new","Baru"],["in_progress","Dalam Proses"],["resolved","Selesai"],["closed","Tutup"]].map(([k,l])=><button key={k} className={"ghost "+(ticketDetail.status===k?"active":"")} disabled={busy} onClick={()=>void setTicketStatus(ticketDetail.id,k as string)}>{l}</button>)}
    </div>
  </div></div>}
  </>}
  </main></div>;
