...
export default function Home() {
  ...
  {view==="transactions"&&<>
    <div className="card"><table className="table"><thead><tr><th>Jenis</th><th>Jumlah</th><th>Vendor</th><th>Pengguna</th><th>Wallet</th><th>Kategori</th><th>Tarikh</th></tr></thead><tbody>{recentTxns.map((t:any)=><tr key={t.id}><td><span className={"pill "+(t.type==="expense"?"bad":"ok")}>{t.type}</span></td><td><b>{Number(t.amount).toLocaleString("ms-MY",{minimumFractionDigits:2})}</b></td><td>{t.vendor_or_source||"—"}</td><td>{t.user_name||t.user_email||"—"}</td><td>{t.wallet_name||"—"}</td><td>{t.category_name||"—"}</td><td>{fmtDate(t.txn_date||t.created_at)}</td></tr>)}</tbody></table></div>
  </>}
