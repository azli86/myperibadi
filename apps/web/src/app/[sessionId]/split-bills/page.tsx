...
export default function SplitBillsPage() {
  ...
  const renderHeroStats = (isDesktop = false) => (
    <div className={cn("splitbill-hero relative overflow-hidden rounded-2xl border border-[var(--border)] bg-[#1a1a1a] text-[#f5f5f5]", isDesktop ? "p-6" : "p-5")}>
      ...
      <div className={cn("relative", isDesktop && "flex items-center gap-5")}>
        <div className={cn(isDesktop && "min-w-[10rem] shrink-0")}>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[0.625rem] font-bold uppercase tracking-[0.1em] text-[#a3a3a3] md:text-[0.6875rem]">
            <span>{tr("Aktif", "Active")}: {activeCount}</span>
            <span>{tr("Selesai", "Done")}: {completedCount}</span>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          {[
            { label: tr("Semua", "All"), value: splits.length },
            { label: tr("Aktif", "Active"), value: activeCount },
            { label: tr("Selesai", "Done"), value: completedCount },
          ].map((item) => (
            <div
              key={item.label}
