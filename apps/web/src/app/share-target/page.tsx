...
import { PENDING_SHARED_CHAT_TOKEN_STORAGE_KEY, SHARED_CHAT_TOKEN_QUERY_KEY, getSharedTransactionPinBypassStorageKey } from "@/lib/share-target"
...
export default function ShareTargetPage() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(PENDING_SHARED_CHAT_TOKEN_STORAGE_KEY, token)
    const accessToken = getAccessToken()
    const refreshToken = getRefreshToken()
    if (!accessToken && !refreshToken) {
      router.replace(`/login?${SHARED_CHAT_TOKEN_QUERY_KEY}=1`)
      return
    }
